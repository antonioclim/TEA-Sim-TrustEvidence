...
class HapiFhirAuditEventClient:
    """Minimal HAPI FHIR REST client used by the External-service conformance harness.

    The class posts AuditEvent resources to a running FHIR endpoint. It is not a
    FHIR-conformance assertion; / remain responsible for exact profiles and
    IG validation.
    """
    ...
    def post_audit_event(self, resource: dict[str, Any]) -> dict[str, Any]:
        if resource.get("resourceType") != "AuditEvent":
            raise ValueError("HAPI client accepts AuditEvent resources only")
        with httpx.Client(timeout=self.timeout_s) as client:
