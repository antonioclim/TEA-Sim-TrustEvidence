...
def replay_local(rows):
    emitter = DeterministicEd25519KeyPair.from_label("urn:te:key:emitter:smoke", "smoke-emitter")
    backend_signer = DeterministicEd25519KeyPair.from_label("urn:te:key:backend:smoke", "smoke-backend")
    a1 = InMemoryA1AppendBackend("urn:te:backend:a1:smoke", backend_signer)
    a2 = A2MerkleLog("urn:te:backend:a2:smoke", "urn:te:log:a2:smoke", backend_signer)
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--results", default="results/smoke_receipts.jsonl")
    args = parser.parse_args(argv)
    rows = load_jsonl(args.workload)
    if args.mode == "external":
        raise SystemExit("external mode requires Docker-composed services; run make smoke after make up on external-service hardware")
