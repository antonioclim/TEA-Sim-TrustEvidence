...
def external_run(workload: Path, hapi_base: str) -> int:
    if shutil.which("docker") is None:
        raise SystemExit("docker not found; external smoke cannot execute in this environment")
    ...
    # service logs can be captured under validation/ by the operator.
    rows = load_jsonl(workload)
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="External smoke harness")
