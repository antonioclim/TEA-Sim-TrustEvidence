...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--profile',default='local',choices=['local','external']); ap.add_argument('--repetitions',type=int,default=10); ap.add_argument('--results-dir',default='results'); ap.add_argument('--require-live',action='store_true'); args=ap.parse_args(); results=Path(args.results_dir); results.mkdir(parents=True,exist_ok=True)
    if args.profile=='external' or args.require_live: raise SystemExit('External benchmarking requires live Docker services; this local package does not fabricate external measurements.')
